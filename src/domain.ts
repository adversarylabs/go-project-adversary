...
export const domain: DomainDefinition = {
  ...
  rules: [
    ...
    },
    {
      ...
      summary: (count) =>
        count === 1
          ? "A mockery verification script can leave obsolete generated mocks undetected."
          : `${count} mockery verification scripts can leave obsolete generated mocks undetected.`,
      whyItMatters:
        "Regenerating known mockery outputs and checking git afterward cannot expose files the generator stopped producing.",
...
/**
 * A mockery verifier must make files that are no longer generated observable.
 * Merely regenerating known outputs before checking git leaves stale tracked
 * mocks untouched, so the check can pass with an obsolete generated file.
 */
export function staleMockeryVerificationSignals(file: SourceRevision): Signal[] {
  if (!isShellScriptLike(file.path) || !isMockVerificationPath(file.path)) return [];

  const lines = file.current.split("\n");
  const generationIndex = lines.findIndex((line) =>
    activeShellLine(line) && invokesMockeryGeneration(line));
  if (generationIndex === -1) return [];

  const verifiesAfterGeneration = lines.slice(generationIndex + 1).some((line) =>
    activeShellLine(line) && /\bgit\b[^#\n]*\b(?:status|diff)\b/.test(line));
  ...
  return [{
    ...
    message:
      "This verifier regenerates mockery output without first removing tracked mockery-generated files, so stale outputs cannot appear as deletions.",
    snippet: generationLine.trim().slice(0, 300),
    data: { generator: "mockery" },
  }];
}

function isMockVerificationPath(path: string): boolean {
  const name = path.split("/").at(-1) ?? path;
  return /(?:verify|check).*(?:mock|generated)|(?:mock|generated).*(?:verify|check)/i.test(name);
}

function activeShellLine(line: string): boolean {
  return !/^\s*(?:#|$)/.test(line);
}

function invokesMockeryGeneration(line: string): boolean {
  return (
    /(?:^\s*|[;&|]\s*|\$\(\s*|\b(?:command|env)\s+)(?:mockery|\$\{?MOCKERY\}?)(?=\s|$)/i.test(line) ||
    /(?:^\s*|[;&|]\s*|\$\(\s*|\b(?:command|env)\s+)make\b[^#\n]*\bmocksgen\b/i.test(line)
  );
}

function hasMarkerScopedMockCleanup(source: string): boolean {
  const hasMarker = /Code generated by mockery/i.test(source);
  const removesMatchedFiles = source.split("\n").some((line) =>
    activeShellLine(line) && /(?:^\s*|[;&|]\s*|\b(?:command|env|sudo|xargs)\s+)rm\b/.test(line));
  return hasMarker && removesMatchedFiles;
}

function invokesExplicitMockCleanup(generationLine: string): boolean {
