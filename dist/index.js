...
function collectInvocations(scope, functions, invoked) {
  ...
  for (let index = 0; index < scope.lines.length; index += 1) {
    ...
    const { text: text2 } = scope.lines[index];
    const active = activeShell(text2);
...
function provesBoundedReadiness(lines, dockerAlias, maximumDepth = 0, requireProcessExit = false) {
  const active = lines.map(({ text: text2 }) => normalizeDockerAlias(activeShell(text2), dockerAlias));
...
function errexitEnabled(source, scope, startIndex) {
  ...
  if (scope.lines.slice(0, startIndex).some(({ text: text2 }, index) => !localDead.has(index) && (localDepths[index] ?? 0) === 0 && enablesErrexit(text2))) return true;
  ...
  return topLevel.lines.some(({ line, text: text2 }, index) => line < scope.startLine && !topDead.has(index) && (topDepths[index] ?? 0) === 0 && enablesErrexit(text2));
}
...
function controlDepths(lines) {
  const depths = [];
  let depth = 0;
  for (const { text: text2 } of lines) {
    const active = activeShell(text2).trim();
