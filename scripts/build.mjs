...
const result = await build({
  ...
  metafile: true,
  banner: {
    js: "import { createRequire as __goProjectCreateRequire } from 'node:module'; const require = __goProjectCreateRequire(import.meta.url);",
  },
});
...
if (staleMappings.length > 0) {
  throw new Error(`stale full license text mapping for unbundled package(s): ${staleMappings.join(", ")}`);
}
